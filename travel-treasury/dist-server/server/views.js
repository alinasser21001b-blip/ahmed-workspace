import { format, toDecimalString, toWire, zero, add } from "../core/money.js";
import { formatRate, rateFromDecimal, rateToDecimalString, rateToWire } from "../core/rate.js";
import { computeCardLedger } from "../core/reconcile.js";
import { treasurySummary } from "../core/treasury.js";
import { rankCards, buildComparison } from "../core/bestcard.js";
import { planWithdrawals } from "../core/planner.js";
import { effectiveRate } from "../core/rate.js";
import { fundingBasisFrom } from "../core/withdrawal.js";
import { evaluateWithdrawal } from "./services.js";
import { cardRefFrom, fundingEventsFor, latestReferenceRate, observationFrom, } from "./repo.js";
import { moneyFrom } from "./repo.js";
import { parseMinor, riyadhDate } from "./util.js";
/**
...
export function evidencedToWire(e) {
    if (!e.known)
        return { known: false, reason: e.reason, missing: e.missing, code: e.code ?? null };
    return {
        known: true,
...
export function evidencedRateToWire(e) {
    if (!e.known)
        return { known: false, reason: e.reason, missing: e.missing, code: e.code ?? null };
    return {
        known: true,
...
async function movementsForTrip(db, tripId) {
    const res = await db.query(`SELECT m.id, w.ownership, m.direction, m.amount_minor, m.kind, m.withdrawal_id, m.expense_id, m.occurred_at, m.notes
    ...
    return res.rows.map((r) => ({
        ...
        amount: { minor: BigInt(r.amount_minor), currency: 'SAR' },
        kind: r.kind,
...
async function evidenceForCards(db, tripId) {
    const cardsRes = await db.query(`SELECT * FROM cards WHERE trip_id = $1 AND is_active ORDER BY created_at`, [tripId]);
    const evidence = [];
    const extras = new Map();
    for (const cardRow of cardsRes.rows) {
        const card = cardRefFrom(cardRow);
        const wdRes = await db.query(`SELECT * FROM withdrawals WHERE card_id = $1 AND state NOT IN ('DRAFT') ORDER BY transaction_at`, [cardRow.id]);
        ...
        for (const row of wdRes.rows) {
            if (row.atm_operator)
                operators.add(row.atm_operator);
            if (row.dcc_selection === 'BILLING_CURRENCY')
                dccUsed += 1;
            for (const f of [row.posted_bank_fee_minor, row.posted_international_fee_minor, row.posted_cash_withdrawal_fee_minor, row.posted_other_fee_minor]) {
                if (f)
                    fees = add(fees, { minor: BigInt(f), currency: card.nativeCurrency });
            }
            if (row.state !== 'RECONCILED')
                continue;
            if (!row.posted_debit_minor || BigInt(row.dispensed_sar_minor) === 0n)
                continue;
            ...
            const dispensed = { minor: BigInt(row.dispensed_sar_minor), currency: 'SAR' };
            const nativeCost = { minor: allIn, currency: card.nativeCurrency };
...
export async function dashboardView(db, tripId, scope) {
    ...
    const counts = await db.query(`SELECT state, count(*)::int AS n, coalesce(sum(dispensed_sar_minor),0)::text AS sar
       FROM withdrawals WHERE trip_id = $1 ${filter} GROUP BY state`, [tripId]);
    const byState = {};
    let totalSar = 0n;
    let count = 0;
    ...
    const feeRes = await db.query(`SELECT posted_debit_currency AS cur,
       ...
       FROM withdrawals WHERE trip_id = $1 ${filter} AND posted_debit_minor IS NOT NULL GROUP BY posted_debit_currency`, [tripId]);
    const openDisc = await db.query(`SELECT count(*)::int AS n FROM discrepancies d JOIN withdrawals w ON w.id = d.withdrawal_id
      WHERE w.trip_id = $1 ${filter.replace('ownership', 'w.ownership')} AND d.user_classification IS NULL`, [tripId]);
    const refs = await db.query(`SELECT DISTINCT ON (base_currency, quote_currency) base_currency, quote_currency, rate::text AS rate, rate_type, effective_date::text AS effective_date
...
export async function cardDashboard(db, cardId) {
    const cardRes = await db.query(`SELECT * FROM cards WHERE id = $1`, [cardId]);
    const cardRow = cardRes.rows[0];
    if (!cardRow)
        return null;
    const card = cardRefFrom(cardRow);
    const wdRes = await db.query(`SELECT * FROM withdrawals WHERE card_id = $1 ORDER BY transaction_at DESC`, [cardId]);
    const snapRes = await db.query(`SELECT * FROM balance_snapshots WHERE card_id = $1 ORDER BY captured_at DESC LIMIT 1`, [cardId]);
    const fundRes = await db.query(`SELECT credited_minor FROM funding_events WHERE card_id = $1`, [cardId]);
    ...
    let todaySar = 0n;
    for (const row of wdRes.rows) {
        if (row.state === 'DRAFT' || row.state === 'REVERSED')
            continue;
...
export async function comparisonView(db, tripId) {
...
export async function plannerView(db, tripId, targetSarMinor, ownership) {
    ...
    for (const cardRow of cards) {
        const card = cardRefFrom(cardRow);
        const snapRes = await db.query(`SELECT * FROM balance_snapshots WHERE card_id = $1 ORDER BY captured_at DESC LIMIT 1`, [cardRow.id]);
        ...
        let referenceNative = null;
        if (!verifiedNative) {
            if (card.nativeCurrency === 'IQD') {
                const ref = await latestReferenceRate(db, 'SAR', 'IQD');
                if (ref)
                    referenceNative = rateFromDecimal(ref.rate, 'SAR', 'IQD');
            }
            else {
                const ref = await latestReferenceRate(db, 'SAR', card.nativeCurrency);
                if (ref)
                    referenceNative = rateFromDecimal(ref.rate, 'SAR', card.nativeCurrency);
            }
        }
        const todaySarRes = await db.query(`SELECT dispensed_sar_minor::text AS t, transaction_at::text AS at FROM withdrawals
        WHERE card_id = $1 AND state NOT IN ('DRAFT','REVERSED','FAILED_ATM')`, [cardRow.id]);
        let withdrawnToday = 0n;
        for (const r of todaySarRes.rows) {
            if (riyadhDate(new Date(r.at).toISOString()) === today)
                withdrawnToday += BigInt(r.t);
        }
        const pendRes = await db.query(`SELECT coalesce(sum(pending_debit_minor + coalesce(pending_fee_minor,0)),0)::text AS p
         FROM withdrawals WHERE card_id = $1 AND posted_debit_minor IS NULL AND pending_debit_minor IS NOT NULL`, [cardRow.id]);
        plannerCards.push({
            card,
...
export async function withdrawalDetail(db, id) {
    const { row, input, computation, reconciliation } = await evaluateWithdrawal(db, id);
    const discRes = await db.query(`SELECT id, expected_minor::text AS expected_minor, observed_minor::text AS observed_minor,
            difference_minor::text AS difference_minor, currency, potential_causes, user_classification, resolution_note
       FROM discrepancies WHERE withdrawal_id = $1 ORDER BY created_at DESC`, [id]);
    const revRes = await db.query(`SELECT field, previous_value, new_value, changed_at::text AS changed_at, reason FROM withdrawal_revisions WHERE withdrawal_id = $1 ORDER BY changed_at`, [id]);
    return {
        id: row.id,
        state: row.state,
        card: input.card,
...
export async function dayCloseView(db, tripId, date) {
    const cardsRes = await db.query(`SELECT * FROM cards WHERE trip_id = $1 AND is_active`, [tripId]);
    const perCard = [];
    for (const cardRow of cardsRes.rows) {
        const dash = await cardDashboard(db, cardRow.id);
        if (!dash)
            continue;
        const wds = await db.query(`SELECT id, state, dispensed_sar_minor::text AS dispensed, transaction_at::text AS at FROM withdrawals WHERE card_id = $1`, [cardRow.id]);
        let daySar = 0n;
        let pendingCount = 0;
        let unsettledCount = 0;
        for (const w of wds.rows) {
            if (riyadhDate(new Date(w.at).toISOString()) === date)
                daySar += BigInt(w.dispensed);
            if (w.state === 'PENDING')
                pendingCount += 1;
            if (['CAPTURED', 'PENDING', 'PARTIAL_DISPENSE', 'DISCREPANCY'].includes(w.state))
                unsettledCount += 1;
        }
        perCard.push({
            cardId: cardRow.id,
    ...
    const movements = await movementsForTrip(db, tripId);
    const treasury = treasurySummary(movements);
    const closeRes = await db.query(`SELECT id, status, closed_at::text AS closed_at FROM day_closes WHERE trip_id=$1 AND close_date=$2`, [tripId, date]);
    return {
        date,
