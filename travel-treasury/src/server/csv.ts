...
import { riyadhDate } from './util.ts';
...
export interface ExportFilters {
  tripId: string;
  ownership?: 'PERSONAL' | 'COMPANY' | null;
  date?: string | null;
  from?: string | null;
  to?: string | null;
}
...
function scaleSar(minor: string | null | undefined): string {
...
function scaleNative(minor: string | null | undefined, currency: string): string {
  if (minor === null || minor === undefined) return '';
  if (currency === 'IQD') return minor;
  return scaleSar(minor);
}

export async function withdrawalsCsv(db: Db, f: ExportFilters): Promise<string> {
  const res = await db.query<Record<string, string | null>>(
    `SELECT w.*, c.nickname, c.issuer, c.product, c.last4, c.native_currency,
      ...
      ORDER BY w.transaction_at`,
    [f.tripId],
  );

  const rows: unknown[][] = [];
  for (const r of res.rows) {
    const localDay = riyadhDate(new Date(r.transaction_at as string).toISOString());
    if (f.date && localDay !== f.date) continue;
    if (f.from && localDay < f.from) continue;
    if (f.to && localDay > f.to) continue;
    const cur = r.native_currency as string;

    let allIn: bigint | null = null;
    let basis = '';
    if (r.posted_debit_minor) {
      ...
      for (const k of ['posted_bank_fee_minor', 'posted_international_fee_minor', 'posted_cash_withdrawal_fee_minor', 'posted_other_fee_minor']) {
        if (r[k]) allIn += BigInt(r[k] as string);
      }
      basis = 'POSTED';
    ...
    const dispensed = BigInt(r.dispensed_sar_minor as string);
    if (allIn !== null && dispensed > 0n) {
      // native major per SAR, to 4 places, exact integer arithmetic.
      const factor = cur === 'IQD' ? 1n : 100n;
      const scaled = (allIn * 100n * 10_000n + (dispensed * factor) / 2n) / (dispensed * factor);
...
export async function reconciliationCsv(db: Db, tripId: string): Promise<string> {
  const res = await db.query<Record<string, string | null>>(
    `SELECT d.id, d.withdrawal_id, w.state, c.nickname, c.native_currency,
       ...
       JOIN cards c ON c.id = w.card_id
      WHERE w.trip_id = $1 ORDER BY d.created_at`,
    [tripId],
  );
  return toCsv(
    ['discrepancy_id', 'withdrawal_id', 'withdrawal_state', 'card', 'currency', 'expected_after_balance', 'observed_after_balance', 'difference', 'cost_basis', 'user_classification', 'resolution_note', 'created_at'],
    res.rows.map((r) => [
      r.id, r.withdrawal_id, r.state, r.nickname, r.currency,
      scaleNative(r.expected_minor, r.currency as string),
      scaleNative(r.observed_minor, r.currency as string),
      scaleNative(r.difference_minor, r.currency as string),
      r.confidence, r.user_classification ?? 'UNCLASSIFIED', r.resolution_note, r.created_at,
    ]),
  );
}

export async function treasuryCsv(db: Db, tripId: string): Promise<string> {
  const res = await db.query<Record<string, string | null>>(
    `SELECT m.id, w.ownership, m.direction, m.amount_minor::text AS amount_minor, m.kind,
  ...
  return toCsv(
    ['movement_id', 'ownership', 'direction', 'amount_sar', 'kind', 'withdrawal_id', 'expense_id', 'occurred_at_utc', 'notes'],
    res.rows.map((r) => [r.id, r.ownership, r.direction, scaleSar(r.amount_minor), r.kind, r.withdrawal_id, r.expense_id, r.occurred_at, r.notes]),
  );
}

export async function auditCsv(db: Db): Promise<string> {
  const res = await db.query<Record<string, string | null>>(
    `SELECT id, actor_user_id, action, entity_table, entity_id,
  ...
  return toCsv(
    ['audit_id', 'actor', 'action', 'entity_table', 'entity_id', 'previous_value', 'new_value', 'reason', 'occurred_at_utc'],
    res.rows.map((r) => [r.id, r.actor_user_id, r.action, r.entity_table, r.entity_id, r.previous_value, r.new_value, r.reason, r.occurred_at]),
  );
}
