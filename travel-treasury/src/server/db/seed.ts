...
import { FINANCIAL_SOURCES } from '../../core/research/sources.ts';
import { SEED_FEE_RULES } from '../../core/research/seed-rules.ts';
 ...
 */
export async function seedResearch(db: Db): Promise<void> {
  await db.transaction(async (tx) => {
    for (const s of FINANCIAL_SOURCES) {
      await tx.query(
        `INSERT INTO financial_sources (id, institution, title, url, source_class, accessed_at, published_at, retrieval_status, notes)
         VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)
         ON CONFLICT (id) DO UPDATE SET institution=$2, title=$3, url=$4, source_class=$5, accessed_at=$6, published_at=$7, retrieval_status=$8, notes=$9`,
        [s.id, s.institution, s.title, s.url, s.sourceClass, s.accessedAt, s.publishedAt, s.retrievalStatus, s.notes],
      );
    }
    for (const r of SEED_FEE_RULES) {
      await tx.query(
        `INSERT INTO fee_rules (id, card_id, issuer, product, rule_type, transaction_type, region,
...
export async function seedUsers(db: Db, users: SeedUserSpec[]): Promise<void> {
  for (const u of users) {
    const existing = await db.query(`SELECT id FROM users WHERE email = $1`, [u.email.toLowerCase()]);
    if (existing.rows[0]) continue;
    const hash = await hashPassword(u.password);
    await db.query(
      `INSERT INTO users (id, email, password_hash, role, display_name, locale) VALUES ($1,$2,$3,$4,$5,'ar')`,
      [newId('user'), u.email.toLowerCase(), hash, u.role, u.displayName],
    );
  }
}
