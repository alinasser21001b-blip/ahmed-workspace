...
import { completeExam } from '@/domain/exam/session';
...
export async function POST(
  ...
  try {
    ...
    const next = completeExam(current, knowledgeView(store));
    await storeRepo.write((s) => {
      const idx = s.sessions.findIndex((row) => row.id === id);
      if (idx >= 0) s.sessions[idx] = next;
    });
    return json({ session: await presentSession(next, { revealAnswers: true }) });
  } catch (error) {
