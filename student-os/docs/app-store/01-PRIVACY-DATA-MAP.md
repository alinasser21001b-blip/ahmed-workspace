...
| Password | sign-up/sign-in | `/v1/auth/{signup,login}` | `users.password_hash` (bcrypt, never the plaintext) | Yes | No | Deleted with the row |
...
## Known gap: EXIF/GPS in uploaded photos

Uploaded images are stored as received. `apps/api/src/modules/files/` was read for this map and does **not** strip EXIF metadata before persisting a photo. A photo taken with location services on and uploaded through `compose.tsx`'s image picker therefore carries GPS coordinates into server storage, silently. This is not disclosed anywhere and is not fixed in this pass — it is a real privacy gap, named here rather than omitted, and should be treated as a follow-up (strip EXIF server-side on ingest, or explicitly disclose it in the privacy policy).
