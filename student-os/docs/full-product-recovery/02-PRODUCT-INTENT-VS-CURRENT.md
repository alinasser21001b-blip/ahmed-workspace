...
| **Profile** | Academic identity and the work behind it; contribution is the only number, and it is "contribution-based, never follower-derived" (`design-handoff/17-PROFILE.md`; `00-PRODUCT-ARCHITECTURE.md:206-210`). Secondary actions include **Message**. | Identity, interests, follow/unfollow, block with confirmation, report, and the author's posts all work. The headline contribution score is `profiles.contribution_score`, created `DEFAULT 0` with a non-negativity check and **never written by any code in the repository** — so in any real deployment it is 0 forever. Non-zero values exist only in preview fixtures. There is no Message action, and no dedicated entry point to one's own profile. | PARTIAL | screen `app/profile/[handle].tsx:123,137,220,243,342`; score column `apps/api/migrations/0002_academic_hierarchy.sql:178,188`; read path `users.repository.ts:45` → `users.service.ts:60` → `app/profile/[handle].tsx:197-207`; fixtures `src/preview/fixtures.ts:249,1161` |
...
|---|---|---|---|---|
| 1 | **A non-zero contribution score** | Preview fixtures fabricate 12 and 120/25. Production renders the DDL default 0 for every user, forever. This is exactly why the always-zero defect survived design review. | PREVIEW_ONLY (the number) / PARTIAL (the feature) | `src/preview/fixtures.ts:249,1161`; column `migrations/0002_academic_hierarchy.sql:178` |
...
3. **One shipped number is false.** The profile contribution score is a `DEFAULT 0` column that no
   code in the repository ever writes (`migrations/0002_academic_hierarchy.sql:178`), rendered as
   the profile's headline figure (`app/profile/[handle].tsx:197-207`). It looked alive only because
   preview fixtures fabricate it (`src/preview/fixtures.ts:249,1161`).
