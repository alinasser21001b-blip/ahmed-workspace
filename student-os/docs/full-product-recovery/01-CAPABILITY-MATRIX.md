...
| `/preview-feedback` in a real build | Was PARTIAL: the route rendered a localized "not available" sentence, but the module still shipped the whole working form, its `recordFeedback` write path, and the banner's own `"— sample data · بيانات تجريبية"` line — an adversarial reviewer caught this as materially the same bug as `/motion-samples`, missed by the first pass. Now excluded the same way the fixture transport is: `metro.config.js` resolves the route and the always-mounted `PreviewBanner` to minimal stubs (a `Redirect` to `/`, and `() => null`) outside a preview export. `e2e/bundle-cleanliness.mjs` asserts `recordFeedback` and the banner string are absent — checked, not assumed | `metro.config.js`, `src/preview/PreviewBanner.stub.tsx`, `src/preview/preview-feedback-route.stub.tsx`; verified absent from a real export by direct string count (`recordFeedback`: 0 in real, 2 in preview) | PHASE_C |
...
| `apps/mobile/dist` on disk can be a stale **preview** export sitting at the production publish path | This directory is **gitignored** (`.gitignore:2`) — nothing here is ever committed, so it carries no repository risk and a fresh `netlify-build.sh` run always regenerates it correctly. The residual risk is local only: a workstation or CI cache that serves a stale `dist/` directly (bypassing the build command) would serve whatever was last exported there. Recorded as a caution for local testing, not a defect in the deployed system | `.gitignore:2` | NONE — nothing in the repository to change |
