...
| CAPABILITY | CLASS | AUDIT STATUS | CODE EVIDENCE | WHAT THE CODE ACTUALLY DOES |
|---|---|---|---|---|
| BLOCK | IMPLEMENTED | CONNECTED_AND_WORKING | `apps/api/src/modules/social/social.routes.ts:75-103`; `apps/api/src/modules/social/social.service.ts:95-118`; `apps/mobile/app/profile/[handle].tsx:112-125` | `PUT /v1/profiles/:handle/block` with an optional reason of ≤500 characters. Self-block is rejected. The block succeeds even when the target is already invisible to the caller. The client shows a confirmation dialogue before applying. **Correction, added after this document was first written**: every individual fact above was true, but the client's `toggleBlock` sent no request body at all, and the server's schema requires the payload to be an object even though `reason` inside it is optional — so every real block attempt failed with 400, silently, because the catch block had no error path. An independent adversarial review found this by clicking through to confirm rather than stopping at the dialog; it is now fixed (`toggleBlock` sends `{}`, matching `toggleBookmark`'s existing pattern) and confirmed live: a bodyless request still 400s, the fixed client's request now returns 200 with `isBlocked:true`, and it appears in `GET /v1/me/blocks`. The lesson generalized: a capability being *wired* is not the same claim as a capability *working*, and this matrix should have said so before an end-to-end write was actually exercised. |
