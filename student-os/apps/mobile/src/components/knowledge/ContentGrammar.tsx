...
export function ContentGrammar({
  ...
  const body = (
    <View style={{ gap: theme.spacing.sm, paddingVertical: theme.spacing.lg }}>
      ...
      {/* 2 — the knowledge body, in the display voice. */}
      {item.body ? (
        <Text
          variant={density === 'detail' ? 'body' : 'bodyStrong'}
          bidi="auto"
          numberOfLines={density === 'feed' ? 5 : density === 'search' ? 3 : undefined}
          style={density !== 'detail' ? { fontWeight: '500' } : undefined}
        >
          {item.body}
        </Text>
      ) : null}
      ...
      {onPressAuthor ? (
        <Pressable
          ...
          hitSlop={6}
        >
          <MetadataLine parts={authorParts} bidi="auto" />
        </Pressable>
      ) : (
        <MetadataLine parts={authorParts} bidi="auto" />
      )}
