...
/**
 * The academic hierarchy: one dense cohort.
 *
 * Lives in `src` rather than in `scripts` because two callers need it and they
 * must not drift: the `db:seed` CLI, and a deployment host that has to bring an
 * empty database up to a state where a student can actually sign up — signup
 * asks for a course, and a database with no courses is an app with no door.
 *
 * Every statement upserts, so running it again is a no-op rather than a
...
export async function seedAcademicHierarchy(): Promise<void> {
